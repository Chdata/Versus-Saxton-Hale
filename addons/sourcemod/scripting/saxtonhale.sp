...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CreateTimer(0.3, MakeHale, _, TIMER_FLAG_NO_MAPCHANGE);
...
{
	CreateTimer(0.1, GottamTimer, _, TIMER_FLAG_NO_MAPCHANGE);
	if (!IsClientInGame(Hale))
	{
		VSHRoundState = VSHRState_End;
		return Plugin_Continue;
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		playing++;
		CreateTimer(0.2, MakeNoHale, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
	}
	//if (playing < 5)
	//  playing += 2;
...
public Action:StartRound(Handle:hTimer)
{
	...
	CreateTimer(10.0, Timer_SkipHalePanel, _, TIMER_FLAG_NO_MAPCHANGE);
	return Plugin_Continue;
}
...
public Action:MakeHale(Handle:hTimer)
{
	...
	CreateTimer(0.2, MakeModelTimer, _, TIMER_FLAG_NO_MAPCHANGE);
...
public Action:Timer_Lazor(Handle:hTimer, any:medigunid)
{
	...
	if (medigun && IsValidEntity(medigun) && VSHRoundState == VSHRState_Active)
	{
		...
		if (IsValidClient(client)) // IsValidClient(client, false)
		{
			...
			if (charge <= 0.05)
			{
				CreateTimer(3.0, Timer_Lazor2, EntIndexToEntRef(medigun), TIMER_FLAG_NO_MAPCHANGE);
...
{
	if (g_bEnabled)
	CreateTimer(0.2, MessageTimer, _, TIMER_FLAG_NO_MAPCHANGE);
	return Plugin_Continue;
}
...
public OnClientDisconnect(client)
{
	...
	if (g_bEnabled)
	{
		if (client == Hale)
		{
			...
			if (VSHRoundState == VSHRState_Waiting)
			{
				...
				if (IsValidClient(tHale))
				{
					Hale = tHale;
					ChangeTeam(Hale, HaleTeam);
					CreateTimer(0.1, MakeHale, _, TIMER_FLAG_NO_MAPCHANGE);
		...
		else
		{
			...
			{
				if (IsPlayerAlive(client)) CreateTimer(0.0, CheckAlivePlayers, _, TIMER_FLAG_NO_MAPCHANGE);
		...
		case VSHSpecial_Vagineer:
			{
				...
				CreateTimer(0.6, UseRage, dist, TIMER_FLAG_NO_MAPCHANGE);
		...
		case VSHSpecial_CBS:
			{
				...
				CreateTimer(0.6, UseRage, dist, TIMER_FLAG_NO_MAPCHANGE);
				CreateTimer(0.1, UseBowRage, _, TIMER_FLAG_NO_MAPCHANGE);
			}
		default:
			{
...
public Action:UseRage(Handle:hTimer, any:dist)
{
	...
	if (!GetEntProp(Hale, Prop_Send, "m_bIsReadyToHighFive") && !IsValidEntity(GetEntPropEnt(Hale, Prop_Send, "m_hHighFivePartner")))
	{
		...
		TF2_StunPlayer(Hale, 0.0, _, TF_STUNFLAGS_LOSERSTATE|TF_STUNFLAG_NOSOUNDOREFFECT, 0); // should reset Hale's animation
	}
	GetEntPropVector(Hale, Prop_Send, "m_vecOrigin", pos);
	for (i = 1; i <= MaxClients; i++)
	{
	...
	while ((i = FindEntityByClassname2(i, "obj_sentrygun")) != -1)
	{
		...
		if (distance < dist)    //(!mode && (distance < RageDist)) || (mode && (distance < RageDist/2)))
		{
			...
			CreateTimer(8.0, EnableSG, EntIndexToEntRef(i), TIMER_FLAG_NO_MAPCHANGE);
		}
	}
	i = -1;
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new customkill = GetEventInt(event, "customkill");
	new weaponindex = GetEventInt(event, "weapon_def_index");
#if defined EASTER_BUNNY_ON
	...
	{
		numHaleKills++;
		if (customkill != TF_CUSTOM_BOOTS_STOMP && weaponindex==5)
		{
		...
		case VSHSpecial_HHH:
			{
				...
				//              CreateTimer(0.1, Timer_ChangeRagdoll, any:GetEventInt(event, "userid"), TIMER_FLAG_NO_MAPCHANGE);
			}
		case VSHSpecial_Hale:
			{
	...
	if (attacker == Hale && VSHRoundState == VSHRState_Active)
	{
		...
		case VSHSpecial_Hale:
			{
				if (customkill != TF_CUSTOM_BOOTS_STOMP && weaponindex==5) SetEventString(event, "weapon", "fists");
		...
		case VSHSpecial_Vagineer:
			{
				...
				//              CreateTimer(0.1, Timer_DissolveRagdoll, any:GetEventInt(event, "userid"), TIMER_FLAG_NO_MAPCHANGE);
			}
		case VSHSpecial_HHH:
			{
...
public Action:HookSound(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (entity == Hale && Special == VSHSpecial_HHH && channel==SNDCHAN_VOICE && StrContains(sample, "halloween_boss") == -1)
	{
	...
	if (Special != VSHSpecial_CBS && channel==SNDCHAN_VOICE && StrContains(sample, "halloween_boss") == -1)
	{
...
{
	if (g_bEnabled && VSHRoundState == VSHRState_Active && StrEqual(classname, "tf_projectile_pipe", false))
...
public OnEggBombSpawned(entity)
{
	...
	RequestFrame(Frame_SetEggBomb, EntIndexToEntRef(entity));
}

public Frame_SetEggBomb(any:ref)
{
...
*/
stock ChangeTeam(iClient, iTeam) // iTeam should never be less than 2 or bigger than 3
{
...
{
	return (GetEntSendPropOffs(iEnt, "m_iTeamNum") <= 0) ? -1 : GetEntProp(iEnt, Prop_Send, "m_iTeamNum");
}
